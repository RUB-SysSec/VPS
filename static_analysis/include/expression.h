...
public:
    ...
    virtual bool propagate(const ExpressionPtr &key,
        ...
        // Prevent circular dependency. We had the following case:
        // key: [(init_rsp - 0x1e8)]
        // value: [([(init_rdi + 0xf)] + [(init_rsp - 0x1e8)])]
        // Since the value also contains, the result would be:
        // [([(init_rdi + 0xf)] + [([(init_rdi + 0xf)] + [...])]
     ...
     * Also `expression & 0xffffffffffffffff` equals `expression`.
     *
    ...
    bool equals_inner(const Expression &other) const {
        ...
        case OperationAnd: {
            if(constant.value() != 0xffffffffffffffff) {
                return *_lhs == other;
            }
            break;
        }
